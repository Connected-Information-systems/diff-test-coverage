...
function parseGlobs(globs, typesOrParsers) {
    return Promise
        .map(globs, (glob, index) => findFilesAndParsers(glob, typesOrParsers, index))
        .map(({files, parser}) => parseFiles(files, parser))
...
function parseFiles(files, typeOrParser) {
    return Promise
        .map(files, file => parseFile(file, typeOrParser))
        .then(filesResults => _.flatten(filesResults))
}

function parseFile(file, typeOrParser) {
