...
function report({coverageByFile, additionsByFile, totals, options}) {
    ...
    if (options.consoleReport.outputLocation) {
        fs.writeFile(options.consoleReport.outputLocation, JSON.stringify(_.values(totals).concat(_.values(coverageByFile))), err => {
            if (err) {
                console.error(err)
...
function getLogMessage({templateName, coverageByFile, additionsByFile, totals, options}) {
    ...
    function separateNonSubsequentLines(file, lineNumber, message){
        if(currentFile !== file){
            currentFile = file;
